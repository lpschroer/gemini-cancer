...
///
/// # Variants
/// - `User` - Content from the user (serializes with `role: "user"`)
/// - `Model` - Content from the model (serializes with `role: "model"`)
/// - `Unspecified` - Content without a role (role field omitted in serialization)
...
pub enum Content<T = String> {
    ...
    /// Content without a specified role (role field omitted)
    #[serde(untagged)]
    Unspecified {
        /// Parts of the content
        parts: Vec<Part<T>>,
...
impl<T> Content<T> {
    /// Creates a new user content with the given parts
    pub fn user(parts: Vec<Part<T>>) -> Self {
        Content::User { parts }
    }

    /// Creates a new model content with the given parts
    pub fn model(parts: Vec<Part<T>>) -> Self {
        Content::Model { parts }
    }

    /// Creates a new unspecified content with the given parts
    pub fn unspecified(parts: Vec<Part<T>>) -> Self {
        Content::Unspecified { parts }
    }

    /// Returns a reference to the parts
    pub fn parts(&self) -> &[Part<T>] {
        match self {
            Content::User { parts } => parts,
            Content::Model { parts } => parts,
            Content::Unspecified { parts } => parts,
        }
    }

    /// Returns a mutable reference to the parts
    pub fn parts_mut(&mut self) -> &mut Vec<Part<T>> {
        match self {
            Content::User { parts } => parts,
            Content::Model { parts } => parts,
            Content::Unspecified { parts } => parts,
    ...
    /// Returns true if this is user content
    pub fn is_user(&self) -> bool {
        matches!(self, Content::User { .. })
    }

    /// Returns true if this is model content
    pub fn is_model(&self) -> bool {
        matches!(self, Content::Model { .. })
    }

    /// Returns true if this is unspecified content
    pub fn is_unspecified(&self) -> bool {
        matches!(self, Content::Unspecified { .. })
    }
...
mod tests {
    ...
    fn test_part_getters() {
        ...
        let part: Part<String> = Part::builder()
            .inline_data(blob)
            .function_call(function_call)
            .build();

        assert!(part.inline_data().is_some());
        assert_eq!(part.inline_data().unwrap().data, "test_data");
        assert!(part.function_call().is_some());
        assert_eq!(part.function_call().unwrap().name, "test_fn");
        ...
        assert!(part.video_metadata().is_none());
    }
}
